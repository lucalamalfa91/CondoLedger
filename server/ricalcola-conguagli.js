 ...
 * il condominio ha speso rispetto al previsto ma non quanto devi tu. Qui dentro
 * c'è il lavoro che riporta i saldi già scritti in banca dati al valore giusto.
 *
 * Lo usano in due: `scripts/ricalcola-conguagli.mjs` da terminale, e
 * `server/bootstrap.js` all'avvio, per gli host dove una shell non c'è.
 */
 ...
 */
export function riscalaRate(splitAmounts, vecchio, nuovo) {
...
/**
 * Guarda tutti i saldi riportati e dice quali sono sbagliati.
 *
 * Non scrive niente: è la parte che si può eseguire sempre, anche per sola
 * curiosità, e che il chiamante decide se trasformare in scrittura.
 */
export async function esaminaConguagli(db) {
  ...
  return { saldi, daCorreggere, saltati };
}

/**
 * Scrive le correzioni, tutte dentro una transazione sola.
 *
 * È idempotente per costruzione: una seconda esecuzione trova i saldi già
 * corretti e non ha più niente da fare. È la proprietà che permette di
 * lasciarla attaccata all'avvio senza doverla rincorrere.
 */
export async function applicaConguagli(db, daCorreggere, log = () => {}) {
  ...
  const esegui = db.transaction(async (tx) => {
    for (const c of daCorreggere) {
      ...
      rateAggiornate += 1;
      log(`  piano rate del ${c.anno}: conguaglio nelle rate ${euro(riscalate.allocato)} → ${euro(c.nuovo)}`);
    }
  });
  await esegui();
  return { rateAggiornate };
}

/** Le due righe che spiegano una correzione, pronte da stampare. */
export function righeCorrezione(c) {
  ...
  return [
    `  ${c.casa} · ${c.anno}  (conguaglio del ${c.anno_origine})`,
    `      ${addendi.join(' + ')} − versato ${euro(c.versato)}`,
    `      ${euro(c.vecchio)}  →  ${euro(c.nuovo)}`
  ];
}
